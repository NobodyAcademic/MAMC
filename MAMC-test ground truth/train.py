...
def train_critics(critics: list[Critic], critic_targets: list[Critic], actors: list[Actor], critic_optimizers: list[torch.optim.Adam], replay_buffer: ReplayBuffer):

    indices = np.random.randint(replay_buffer.size, size=(args.critic_size, args.batch_size))

    for i in range(args.smr_ratio):

        calculate_target_Q(critic_targets, actors, replay_buffer, indices)
...
def calculate_target_Q(critic_targets: list[Critic], actors: list[Actor], replay_buffer: ReplayBuffer, indices: np.ndarray):
