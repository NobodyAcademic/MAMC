...
exploration_noise_tensor = torch.tensor(exploration_noise, dtype=torch.float32, device=args.device)
max_action_tensor = torch.tensor(max_action, dtype=torch.float32, device=args.device)
...
for steps in range(args.max_steps):
    ...
    if steps < args.start_steps:
        ...
        train_critics(critics, critic_targets, actors, critic_optimizers, replay_buffer, exploration_noise_tensor, max_action_tensor)
